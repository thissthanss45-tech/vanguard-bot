...
| Категория | Описание |
|---|---|
| **Теханализ** | SMA20/50, EMA200, RSI14, ATR14, ADX14, ADX-серия, Bollinger Bands, Stochastic, OBV, MACD, Parabolic SAR, S/R уровни |
| **Прогноз** | Rule-based модель: вероятность роста/снижения, quality gate, D1/D2/D3; адаптивные веса по типу инструмента (крипта/акции/форекс/фьючерсы) |
| **SL/TP** | ATR-based уровни Stop Loss и Take Profit с профилями риска (conservative/moderate/aggressive) |
| **Intermarket** | Корреляции с `SPY` и `BTC-USD` |
| **Fear & Greed** | Умный источник: крипта → alternative.me, акции → CNN; автоматический fallback |
...
| **Портфель** | Трекер сделок: добавление через кнопки-мастер, закрытие с P&L (USD + %), история закрытых позиций |
| **Вотчлист** | Дайджест-сканирование по расписанию, уведомления по алертам, кэш |
| **Бэктест** | Точность прогнозов по тикеру из вотчлиста через кнопки, общая таблица |
| **Сравнение** | `/compare TICK1 TICK2` — side-by-side теханализ двух инструментов |
| **Персистентность** | `PicklePersistence` — алерты и вотчлист сохраняются после рестарта |
| **Кэш** | `memory` / `diskcache` / `redis` |
| **Графики** | PNG-чарт с SMA/EMA в Telegram |
| **UX** | Категории тикеров, пагинация, мультиязычность (ru/en), кнопочные меню |
...
| `/compare AAPL MSFT` | Сравнить два инструмента |
| `/trade add\|close\|list` | Управление сделками |
| `/portfolio` | P&L портфеля |
| `/backtest [TICKER]` | Точность прогнозов по тикеру |
...
├── portfolio_tracker.py     # Трекер сделок: P&L, история закрытых позиций
